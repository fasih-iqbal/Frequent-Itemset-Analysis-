...
# Collection for product purchases
purchases_collection = db['product_purchases']
    ...
    if isinstance(data, list):
        ...
        for product_id, trends in trend_data.items():
            if len(trends['views']) == window_size and len(trends['purchases']) == window_size:
                ...
                print(f"Product {product_id} has an average of {
                      avg_views:.2f} views and {avg_purchases:.2f} purchases.")

                # Save data to MongoDB
                views_data = {'product_id': product_id,
                              'average_views': avg_views}
                purchases_data = {'product_id': product_id,
                ...
                if avg_views / max(avg_purchases, 1) > 10:
                    print(f"Warning: High view-to-purchase ratio for {
                          product_id}. Consider checking stock levels.")
                if avg_purchases / max(avg_views, 1) > 0.5:
                    print(
                        f"Alert: High purchase-to-view ratio for {product_id}. Potential stockout risk.")
